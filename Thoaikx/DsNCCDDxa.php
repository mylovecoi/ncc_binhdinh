		...
		<tr>
			<td style="text-align: left;font-weight: bold;"><nobr><?php
					if (substr($tenhuyen,0, 2) == "TP")
						echo "ỦY BAN NHÂN DÂN ".strtoupper($tenhuyen);
					else if (substr($tenhuyen,0, 2) == "Thị Xã")
						echo "ỦY BAN NHÂN DÂN ".strtoupper($tenhuyen);
					else
						echo "ỦY BAN NHÂN DÂN HUYỆN ".strtoupper($tenhuyen);
				?></nobr></td>
			<td>
			</td>
...
$sql = "Select hoten,truquan,doituong,namddlt,tinhtrangsk,phanloaidd,sotien from dieuduong WHERE namdd = '$nambc' and chuyen = 'Duyệt' and loaidd = 'Một năm một lần' and xa ='".$tenxa."' order by hoten";
...
//." order by hoten";
$sql = "Select hoten,truquan,doituong,namddlt,tinhtrangsk,phanloaidd,sotien from dieuduong WHERE namdd = '$nambc' and chuyen = 'Duyệt' and loaidd = 'Hai năm một lần' and xa='".$tenxa."' order by hoten";
$qrsql = mysqli_query($con,$sql);
$cc = array();
while ($row = mysqli_fetch_array($qrsql)) {		
		$stt++;	
