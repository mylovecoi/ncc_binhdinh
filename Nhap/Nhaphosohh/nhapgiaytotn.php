...
<script type="text/javascript" language="javascript" class="init">
			...
			{  
			   ...
			   { 					
					...
					$("#idgiaytohhtncs").val($(this).find('td:nth-child(5)').text());
					$("#idthannhanhhcs").val($(this).find('td:nth-child(6)').text());					
			   }); 
			}
...
if(isset($_POST["capnhatts"]))
	{		
		$sqlup = "Update giaytohhtn Set tengiayto='".$_POST['tengiaytocs']."', chitiet='".$_POST['chitietcs']."', soluong=".kieudouble($_POST['soluongcs']);		
		$sqlup = $sqlup." Where idgiaytohhtn=".$_POST['idgiaytohhtncs'];
		//echo $sqlup;	
		$kq=mysqli_query($con,$sqlup);
	}
if(isset($_POST["themmoi"]))
	{		
		$sqlup = "Insert into giaytohhtn Set tengiayto='".$_POST['tengiayto']."', chitiet='".$_POST['chitiet']."', soluong=".kieudouble($_POST['soluong']);		
		$sqlup = $sqlup.", idthannhanhh=".$ihs;
		//echo $sqlup;	
		$kq=mysqli_query($con,$sqlup);
	}	
?>
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Danh sách giấy tờ thân nhân hồ sơ hóa học 
		...
		<table class="table table-hover table-striped table-bordered table-advanced tablesorter display" id="table-4">
			<thead>
				<tr>
					...
					<th style="background-color:rgb(16, 78, 211); display:none" width="30%"><strong>idgiaytols</strong></th>
					<th style="background-color:rgb(16, 78, 211); display:none" width="30%"><strong>idhsls</strong></th>
				</tr>				
			</thead>
	<?php		
		$sqlgt = "Select * From giaytohhtn Where idthannhanhh = $ihs order by chitiet";	
		$i2 = 0;	
		...
		while($rctgt=@mysqli_fetch_array($qrctgt))
			{	
				...
				echo "<tr>";
					...
					echo "<td style='text-align: left; display:none'>".$rctgt['idgiaytohhtn']."</td>";
					echo "<td style='text-align: left; display:none'>".$rctgt['idthannhanhh']."</td>";
				echo "</tr>";	
			}		
			echo "</tbody>";
	?>		
			...
			<div class="modal-body">				
					<form name="thoaics" role='form' method='POST' class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK" class="form-group">							
							...
							<div class="col-sm-10">
								<input name="idgiaytohhtncs" type="text" class="form-control" style="display:none" id="idgiaytohhtncs">
							</div>
							<div class="col-sm-10">
								<input name="idthannhanhhcs" type="text" class="form-control" style="display:none" id="idthannhanhhcs">
							</div>
						</div>
							<button type="button" class="btn btn-black" data-dismiss="modal">Đóng</button>
					<?php	
...
<script type="text/javascript">
    ...
    function get_action() {				
			return "nhapgiaytotn.php?id=" + $('#idthannhanhhcs').val();			
    }
