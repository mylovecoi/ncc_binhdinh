...
{	
	...
	for($i=0;$i<$count;$i++) {
		...
		if ($_POST['ngayhuyenchuyen'] != "") {
			$sql = "Update dieuduong Set chuyenhuyen = 'Chuyển tỉnh', chuyen='Chuyển 2',ngayhuyen='" . doingay1($_POST['ngayhuyenchuyen']) . "'";
			if ($_POST['dotddchuyen'] != "")
				$sql = $sql . ",dotdd='" . $_POST['dotddchuyen'] . "'";
			$sql = $sql . " Where iddieuduong=" . $key[$i];
		} else {
			$sql = "Update dieuduong Set chuyenhuyen = 'Chuyển tỉnh', chuyen='Chuyển 2'";
			if ($_POST['dotddchuyen'] != "")
				$sql = $sql . ",dotdd='" . $_POST['dotddchuyen'] . "'";
			$sql = $sql . " Where iddieuduong=" . $key[$i];
		}
...
if (isset($_POST['chuyenxa'])) {
	...
	for ($i = 0; $i < $count; $i++) {
		//echo $madv[0];
		$sql = "";
			$sql = "Update dieuduong Set chuyenhuyen = 'Chưa', chuyen='',lydochuyenxa = '" . $_POST['lydochuyen'] . "' , huyentra = '1'";
				$sql = $sql . ",dotdd=''";
			$sql = $sql . " Where iddieuduong=" . $key[$i];
		//echo $sql;
		$kq = mysqli_query($con, $sql);
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Lập danh sách hồ sơ điều dưỡng</h4>
		</div>
...
<form role='form' method='POST' class='form-horizontal form-groups-bordered'>								
		<a style = "float:right; background-color:rgb(83, 181, 166)" onclick="showAjaxModaldc();" class='btn btn-success'>
			<i class='entypo-print'></i>Lập danh sách
		</a>
...
$j = $ahs[0];
	...
	$cs=0;
	$sql="select * from dieuduong where (chuyen='Chuyển 1' or chuyenhuyen = 'Đã chuyển') and huyen='".$madv[0]."' ".
		" and xa like '%" . str_replace('_', ' ', $ahs[2]) . "%' and tinhtra <> 1 order by xa";
...
?>
	...
	<div id = "hsls" class="table-responsive">
		...
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title">Danh sách Đợt chuyển lên cấp trên</h4>
			</div>
		<div id = "" >
			<table class="table table-bordered datatable" >
				...
				<tr>
					...
					<th style="background-color:rgb(54, 169, 224)" width="15%"><strong style='color:rgb(255, 255, 225)'><p>Ngày chuyển</p></strong></th>
					<th style="background-color:rgb(54, 169, 224)" width="15%"><strong style='color:rgb(255, 255, 225)'><p>Năm điều dưỡng</p></strong></th>
					<th style="background-color:rgb(54, 169, 224)" width="48%"><strong style='color:rgb(255, 255, 225)'><p>Đợt chuyển</p></strong></th>
				</tr>
				</thead>
				<?php
				$sql = "Select distinct dotdd,ngayhuyen,namdd from dieuduong WHERE dotdd <> 0 and (chuyen = 'Chuyển 2' or chuyen = 'Duyệt')".
				" and huyen = '".$madv[0]."'";
				//echo $sql;
				...
				while($row = mysqli_fetch_array($qrsql)) {
					...
					echo "<td>" . ngaythang($row['ngayhuyen']) . "</td>";
					echo "<td>" .$row['namdd'] . "</td>";
					echo "<td>" . $row['dotdd'] . "</td>";
					echo "</tr>";
				}
				?>
			</table>
