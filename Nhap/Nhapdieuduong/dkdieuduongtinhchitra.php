...
$(document).ready(function() {
  $('#trang').change(function() {
		...
		if ($("#dkien").val().length > 15)
			window.location.href="dkdieuduongtinhchitra.php?id="+this.value+$("#dkien").val().substr(1);
		else
			window.location.href="dkdieuduongtinhchitra.php?id="+this.value+">0";
  });
 });
</script>
...
{
	$sql="select * from dieuduong where namdd Like '".$ahs[1]."%' and chuyen='Duyệt' and xa like '%".str_replace('_',' ',$ahs[13])."%' and huyen Like '%".str_replace('_',' ',$ahs[12])."%'";
	if ($ahs[2] != "")
	...
	if ($dkkhac != "")
		$sql=$sql." and (".$dkkhac.")";
	$sql = $sql. "order by huyen";
}
else
{
	$sql="select * from dieuduong where chuyen='Duyệt'";
}
echo $sql;
		...
		<table class="table table-hover table-striped table-bordered table-advanced tablesorter display" id="table-2">
			<thead>
				<tr>					
					...
					<th style="background-color:rgb(54, 169, 224)" width="8%"><strong style='color:rgb(255, 255, 225)'></th>
				</tr>				
			</thead>
...
<?php
			...
			for($i=($j - 1)*15;$i<15*$j;$i++)
			{
				...
				echo "<tr>";					
					...
					echo "<td>
						<a href=\"ChuyenHS.php?id=$chuyen\"  title='Chuyển hồ sơ về huyện' onclick=\"return confirm('Bạn có muốn chuyển hồ sơ điều dưỡng này về huyện?')\" class='btn btn-red '>
							</i>
							Chuyển HS</a>
                    </td>";
				echo "</tr>";
...
<script type="text/javascript">
	...
	giatri="1>"+$('#namdd').val()+">"+b1+">"+b2+">"+b3+">"+b4+">"+b5+">"+b6+">"+b7+">"+b8+">"+b9+">"+b10+">"+$('#tenhuyentao').val()+">"+$('#tenxatao').val();					
			return "dkdieuduongtinhchitra.php?id="+giatri;			
    }
