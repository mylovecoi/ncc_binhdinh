...
{	
	...
	for($i=0;$i<$count;$i++){
		//echo $madv[0];
	$sql="Update dieuduong Set chuyen='Duyệt', lydokhongduyet='' Where iddieuduong=".$key[$i];
	$kq=mysqli_query($con,$sql);
	}
}		
...
{	
	...
	for($i=0;$i<$count;$i++){
		//echo $madv[0];
	$sql="Update dieuduong Set chuyen='Chuyển 1',lydokhongduyet='".$_POST['lydokd']."',tinhtra = '1'  Where iddieuduong=".$key[$i];
	$kq=mysqli_query($con,$sql);
	}
}
$dieukien=$_GET['id'];
...
{
	$sql="select * from dieuduong where namdd Like '".$ahs[1]."%' and chuyen='Chuyển 2' and xa like '%".str_replace('_',' ',$ahs[13])."%' and huyen Like '%".str_replace('_',' ',$ahs[12])."%'";
	if ($ahs[2] != "")
...
{
	$sql="select * from dieuduong where chuyen='Chuyển 2'";
}
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Duyệt danh sách hồ sơ điều dưỡng</h4>
		</div>
		...
		<div class="modal-content">
			<div class="modal-body">				
					<form name="thoaichuyen" role='form' method='POST' enctype="multipart/form-data" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK" class="form-group">
							<h4 style = "text-align:center; color:rgb(10, 10, 10)">DUYỆT DANH SÁCH ĐIỀU DƯỠNG</h4>	
							<h4 style = "text-align:center; color:rgb(10, 10, 10)">(Lưu ý)</h4>
							<label for="field-3" class="col-sm-12 control-label" style = "text-align: left;">-   Duyệt danh sách sẽ không chỉnh sửa được</label>
		...
		<div class="modal-content">
			<div class="modal-body">				
					<form name="thoaichuyenx" role='form' method='POST' enctype="multipart/form-data" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK" class="form-group">
							<h4 style = "text-align:center; color:rgb(10, 10, 10)">KHÔNG DUYỆT DANH SÁCH</h4>	
							<h4 style = "text-align:center; color:rgb(10, 10, 10)">(Lưu ý)</h4>
							<label for="field-3" class="col-sm-12 control-label" style = "text-align: left;">-   Nêu rõ lý do không duyệt hồ sơ</label>
							<input name="sobg1x" type="text" class="form-control" style = "Display:none;" id="sobg1x">
							<label for="field-3" class="col-sm-3 control-label">Lý do không duyệt:</label>
							<div class="col-sm-9">
								<input name="lydokd" type="text" class="form-control" id="lydokd">
							</div>
						</div>
							<button type="button" class="btn btn-black" data-dismiss="modal">Đóng</button>
							<input type="submit" name="chuyenx" class="btn btn-blue" value = "Cập nhật" id = "hhhh1x">							
					</form>						
			</div>
		</div>
	</div>
