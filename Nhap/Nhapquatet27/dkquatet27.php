...
$(document).ready(function() {
  $('#trang').change(function() {
		...
		window.location.href="dkquatet27.php?id="+this.value+">0";
  });
 });
</script>
...
if(isset($_POST["themmoi"]))
	{	
		...
		$sqlup = $sqlup.", huyen='".$madv[0]."', xa='".str_replace('_',' ',$_POST['xahuyen'])."', chuyen='Duyệt'";
		$kq=mysqli_query($con,$sqlup);
	}
...
{	
	...
	if (isset($_POST['tnls']))
	{
		...
		$sql = $sql.", 'Duyệt' as dk, xa, huyen, tenncc";
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Lập danh sách hồ sơ hưởng quà tết, 27/7</h4>
		</div>
			<div class="modal-body">
<form role='form' method='POST' class='form-horizontal form-groups-bordered'>								
		<a style = "float:right; background-color:rgb(83, 181, 166)" onclick="showAjaxModaldc();" class='btn btn-success'>
			<i class='entypo-print'></i>Lập DS
		</a>	
<?php	
		//echo "<a style = 'float:right; background-color:rgb(83, 181, 166)' onclick='showAjaxModalcds();' class='btn btn-success'>
			//<i class='entypo-print'></i>Chuyển DS
		//</a>";
?>
		...
		<a style = "float:right; background-color:rgb(83, 181, 166)" onclick="showAjaxModalcthem();" class='btn btn-success'>
			<i class='entypo-print'></i>Thêm DS
		</a>
...
if (strlen($_GET['id'])<8)
{
	$sql="select * from quatet27 where chuyen='Duyệt' and huyen='".$madv[0]."' and namqua Like '%".$ahs[1]."%'";
}
else
{
	$sql="select * from quatet27 where chuyen='Duyệt' and huyen='".$madv[0]."' and namqua Like '".$ahs[3]."%' and loaidoituong Like '".$ahs[1]."%' and mucqua=".str_replace(',','',str_replace('.','',$ahs[2]))." and xa like '".str_replace('_',' ',$ahs[4])."%'";
}
	...
	return $kq;
			...
			for($i=($j - 1)*15;$i<15*$j;$i++)
			{
				...
				if ($ls[$i][2]=="lt")		
					...
					echo "<td style='text-align: left;'>".catchuoi('Thân nhân liệt sĩ đang hưởng trợ cấp hàng tháng')."</td>";
					...
					echo "<td style='text-align: left;'>".catchuoi('Người có công giúp đỡ cách mạng đang hưởng trợ cấp nuôi dưỡng hàng tháng')."</td>";
				else if ($ls[$i][2]=="cc")		
					echo "<td style='text-align: left;'>".catchuoi('Người có công giúp đỡ cách mạng đang hưởng trợ cấp hàng tháng')."</td>";
				else if ($ls[$i][2]=="hhdb")		
					...
					echo"<td>					
							<a href=\"xoa.php?id=".$ls[$i][0]."\"  title='Xóa hố sơ nhận quà' onclick=\"return confirm('Bạn muốn xóa hố sơ này?')\" class='btn btn-red '>
								</i>
								Xóa
							</a>
						</td>";
			...
			<div class="modal-body">				
					<form name="thoaics" role='form' method='POST' enctype="multipart/form-data" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK" class="form-group">
							...
							<label for="field-3" class="col-sm-3 control-label">Mức trợ cấp:</label>
							<div class="col-sm-9">
								<input name="mucquacs" type="text" class="form-control" data-mask="fdecimal" id="mucquacs">
							</div>
							<label for="field-3" class="col-sm-3 control-label">Năm trợ cấp:</label>
							<div class="col-sm-9">								
			...
			<div class="modal-body">				
					<form name="thoaitao" role='form' method='POST' class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK1" class="form-group">								
							<label for="field-3" class="col-sm-2 control-label">Năm nhận quà:</label>
							<div class="col-sm-4">
							...
							<div class="col-sm-10">								
								<select name="noidungtao" id="noidungtao" class="form-control">
									<option value="Quà tết nguyên đán">Quà tết nguyên đán</option>
									<option value="Quà 27/7">Quà 27/7</option>
								</select>
							</div>	
...
<script type="text/javascript">
	...
	function get_action1() {				
			return "dkquatet27.php?id="+"1>"+$('#namqua').val();			
    }
	function get_action2() {				
			return "dkquatet27.php?id=1>"+$('#namquatao').val();			
    }
	function get_action3() {				
			return "dkquatet27.php?id=1>"+$('#loaidoituongtim').val()+">"+$('#mucquatim').val()+">"+$('#namquatim').val()+">"+$('#xatim').val();			
    }
	function get_action4() {				
			return "danhsachqua.php";
    }	
</script>
<!------ FOOTER --------->
