...
$(document).ready(function() {
  $('#trang').change(function() {
		...
		window.location.href="dkquatet27tinh.php?id="+this.value+">0";
  });
 });
</script>
...
if(isset($_POST["themmoi"]))
	{	
		...
		$sqlup = $sqlup.", huyen='".$madv[0]."', xa='".str_replace('_',' ',$_POST['xahuyen'])."', chuyen='Chuyển'";
		$kq=mysqli_query($con,$sqlup);
	}
...
{	
	...
	for($i=0;$i<$count;$i++){
		//echo $madv[0];
	$sql="Update quatet27 Set chuyen='Duyệt' Where idquatet27=".$key[$i];
	$kq=mysqli_query($con,$sql);
	}
}		
?>
...
if (strlen($_GET['id'])<8)
{
	$sql="select * from quatet27 where chuyen='Chuyển' and namqua Like '%".$ahs[1]."%'";
}
else
{
	if ($ahs[2]=="")
		$sql="select * from quatet27 where chuyen='Chuyển' and namqua Like '".$ahs[3]."%' and loaidoituong Like '".$ahs[1]."%' and xa like '".str_replace('_',' ',$ahs[4])."%'";
	else
		$sql="select * from quatet27 where chuyen='Chuyển' and namqua Like '".$ahs[3]."%' and loaidoituong Like '".$ahs[1]."%' and mucqua=".str_replace(',','',str_replace('.','',$ahs[2]))." and xa like '".str_replace('_',' ',$ahs[4])."%'";
}
//echo $sql;
...
function catchuoi1($chuoi)	
{
	...
	return $kq;
			...
			for($i=($j - 1)*15;$i<15*$j;$i++)
			{
				...
				{					
				echo "<tr>";					
					echo "<td><input type = 'Checkbox' name = 'KT' id = '".$ls[$i][0]."'></td>";
			...
			<div class="modal-body">				
					<form name="thoaitao" role='form' method='POST' class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK1" class="form-group">								
							<label for="field-3" class="col-sm-2 control-label">Năm nhận quả:</label>
							<div class="col-sm-4">
							...
							<label for="field-3" class="col-sm-2 control-label">Nội dung nhận quả:</label>
							<div class="col-sm-10">
...
<script type="text/javascript">
	...
	function get_action1() {				
			return "dkquatet27tinh.php?id="+"1>"+$('#namqua').val();			
    }
	function get_action2() {				
			return "dkquatet27tinh.php?id=1>"+$('#namquatao').val();			
    }
	function get_action3() {				
			return "dkquatet27tinh.php?id=1>"+$('#loaidoituongtim').val()+">"+$('#mucquatim').val()+">"+$('#namquatim').val()+">"+$('#xatim').val();			
    }
	function get_action4() {				
			return "danhsachquatinh.php";
    }	
</script>
<!------ FOOTER --------->
