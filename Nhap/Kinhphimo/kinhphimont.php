...
<script type="text/javascript" language="javascript" class="init">
			...
			{  
			   $('table tbody tr').dblclick(function() 
			   {
					...
					$("#khucs").val($(this).find('td:nth-child(19)').text());
					$("#hangcs").val($(this).find('td:nth-child(20)').text());
					$("#mocs").val($(this).find('td:nth-child(21)').text());
					$("#tennghiatrangcs").val($(this).find('td:nth-child(22)').text());
					$("#noiquytapcs").val($(this).find('td:nth-child(24)').text());
			   }); 
			}
...
$(document).ready(function() {
  $('#trang').change(function() {
		window.location.href="kinhphimont.php?id="+this.value+">"+$("#sobanghixx").val();//+">"+$("#hotenxx").val()+">"+$("#ngquanxx").val()+">"+$("#thanhanxx").val()+">"+$("#chooxx").val()+">timkiem";		
  });
 });		
</script>
<?php
...
if(isset($_POST["capnhatts"]))
	{
		if ($madv[3]=="Tỉnh")
		{
			$sql1 = "Select idnghiatrang,tennghiatrang From nghiatrang Where idnghiatrang=".$_POST['tennghiatrangcs'];			
			...
			$sqlup = $sqlup."capbacchucvu='".$_POST['capbacchucvucs']."',sosols='".$_POST['sosolscs']."', sobangls='".$_POST['sobanglscs']."', thannhan='".$_POST['thannhancs']."', tennghiatrang='".$ten."',";
			$sqlup = $sqlup."ghichu='".$_POST['ghichucs']."',khu='".$_POST['khucs']."',hang='".$_POST['hangcs']."',mo='".$_POST['mocs']."',idnghiatrang=".$_POST['tennghiatrangcs'].",noiquytap='".$_POST['noiquytapcs']."',";
...
if(isset($_POST["themmoi"]))
{
	if ($madv[3]=="Tỉnh")
	{
		$sql1 = "Select idnghiatrang,tennghiatrang From nghiatrang Where idnghiatrang=".$_POST['tennghiatrang'];
		$qrt1 = mysqli_query($con,$sql1);	$ten="";		
		while($rt1=@mysqli_fetch_array($qrt1))
		{																			
			$ten = $rt1['tennghiatrang'];
		...
		$sqlup = $sqlup."capbacchucvu='".$_POST['capbacchucvu']."',sosols='".$_POST['sosols']."', sobangls='".$_POST['sobangls']."', thannhan='".$_POST['thannhan']."', tennghiatrang='".$ten."',";
		$sqlup = $sqlup."ghichu='".$_POST['ghichu']."',khu='".$_POST['khu']."',hang='".$_POST['hang']."',mo='".$_POST['mo']."',idnghiatrang=".$_POST['tennghiatrang'].",noiquytap='".$_POST['noiquytap']."',";
		$sqlup = $sqlup."quanhe='".$_POST['quanhe']."', choo='".$_POST['choo']."', tongtien=".str_replace(',','',str_replace('.','',$_POST['tongtien'])).", ";
		$sqlup = $sqlup."xa='".str_replace('_',' ',$xh[1])."', huyen='".str_replace('_',' ',$xh[0])."',phanloai='NT'";		
		//echo $sqlup;	
		$kq=mysqli_query($con,$sqlup);
	}
}	
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Hồ sơ kinh phí xây dựng mộ trong nghĩa trang liệt sỹ <span style = "float:right">Tổng hồ sơ: <?php echo $ahs[1];?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></h4>
		</div>
...
if (strlen($dkkhac) > 12)
{
	$sql="select * from kinhphimo where phanloai='NT' and hotenls Like '".$ahs[2]."%' and nguyenquan like '%".$ahs[3]."%' and thannhan like '%".$ahs[4]."%' and choo Like '%".$ahs[5]."%'";	
...
{
	$sql="select * from kinhphimo Where phanloai='NT' LIMIT ".(15*($j-1)).",15";
...
<th style="background-color:rgb(16, 78, 211); display:none" width="30%"><strong>mo</strong></th>	
<th style="background-color:rgb(16, 78, 211); display:none" width="30%"><strong>idnghiatrang</strong></th>	
<th style="background-color:rgb(16, 78, 211); display:none" width="30%"><strong>tennghiatrang</strong></th>	
<th style="background-color:rgb(16, 78, 211); display:none" width="30%"><strong>noiquytap</strong></th>						
				</tr>				
			</thead>
...
<?php
	...
	while($row=mysqli_fetch_array($sql1)){
				...
				echo "<tr>";					
					...
					echo"<td>					
							<a href=\"xoant.php?id=".$row['idkpmo']."-".$ahs[1]."\"  title='Xóa hố sơ mộ liệt sỹ' onclick=\"return confirm('Bạn muốn xóa hố sơ này?')\" class='btn btn-red '>
								</i>
								Xóa
							</a>
						</td>";
...
echo "<td style='text-align: left; display:none'>".$row['khu']."</td>";
echo "<td style='text-align: left; display:none'>".$row['hang']."</td>";
echo "<td style='text-align: left; display:none'>".$row['mo']."</td>";
echo "<td style='text-align: left; display:none'>".$row['idnghiatrang']."</td>";
echo "<td style='text-align: left; display:none'>".$row['tennghiatrang']."</td>";
echo "<td style='text-align: left; display:none'>".$row['noiquytap']."</td>";					
				echo "</tr>";								
			}
?>				
			</tbody>
			...
			<div class="modal-body">				
					<form name="thoaimoi" role='form' method='POST' class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK1" class="form-group">
							...
							<label for="field-3" class="col-sm-2 control-label">Khu mộ:</label>
							<div class="col-sm-4">
								<input name="khu" type="text" class="form-control" id="khu">
							</div>
							<label for="field-3" class="col-sm-2 control-label">Hàng mộ:</label>
							<div class="col-sm-4">
								<input name="hang" type="text" class="form-control" id="hang">
							</div>
							<label for="field-3" class="col-sm-2 control-label">Số mộ:</label>
							<div class="col-sm-4">
								<input name="mo" type="text" class="form-control" id="mo">
							</div>
							...
							<div class="col-sm-10">								
								<select name="tennghiatrang" id="tennghiatrang" class="form-control">
									<?php
							...
							<label for="field-3" class="col-sm-2 control-label">Nơi quy tấp:</label>
							<div class="col-sm-10">
								<input name="noiquytap" type="text" class="form-control" id="noiquytap">
							</div>	
			...
			<div class="modal-body">				
					<form name="thoaics" role='form' method='POST' enctype="multipart/form-data" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK" class="form-group">
							...
							<label for="field-3" class="col-sm-2 control-label">Số mộ:</label>
							<div class="col-sm-4">
								<input name="mocs" type="text" class="form-control" id="mocs">
							</div>
							<label for="field-3" class="col-sm-2 control-label">Họ tên thân nhân:</label>
							<div class="col-sm-4">
							...
							<label for="field-3" class="col-sm-2 control-label">Tên nghĩa trang:</label>
							<div class="col-sm-10">								
								<select name="tennghiatrangcs" id="tennghiatrangcs" class="form-control">
									<?php
									$sqltcml = "Select idnghiatrang,tennghiatrang From nghiatrang";
									$qrtcml = mysqli_query($con,$sqltcml);			
									while($rtcml=@mysqli_fetch_array($qrtcml))
							...
							<label for="field-3" class="col-sm-2 control-label">Nơi quy tấp:</label>
							<div class="col-sm-10">
								<input name="noiquytapcs" type="text" class="form-control" id="noiquytapcs">
							</div>	
							<label for="field-3" class="col-sm-2 control-label">Tên xã:</label>
							<div class="col-sm-10">								
...
<div class="modal fade" id="modal-2">
	<div class="modal-dialog">
		<div class="modal-content">				
			<div class="modal-body">				
					<form name="thoaiin" role='form' method='POST' action="reportmont.php" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK1" class="form-group">
...
<script type="text/javascript">
    ...
    function get_action() {				
			return "kinhphimont.php?id=" + $('#trang').val() + ">" + $('#sobanghixx').val();			
    }
	function get_action1() {				
			return "kinhphimont.php?id=1>"+ $('#sobanghixx').val();
	}
	function get_action2() {				
			return "kinhphimont.php?id="+"1>"+ $('#sobanghixx').val()+">"+$('#hotentim').val()+">"+$('#ngquantim').val()+">"+$('#thannhantim').val()+">"+$('#chootim').val()+">timkiem";		
    }
</script>
<!------ FOOTER --------->
