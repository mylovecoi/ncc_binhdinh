...
{		
	window.open("/Reports/Phieudoichieubmt.php?id="+$("#soghi").val()+">"+$("#hotendc").val()+">"+$("#ngaysinhdc").val()+">"+$("#nguyenquandc").val()+">"+$("#truquandc").val()+">"+$("#mahsbodc").val()+">"+$("#mahstinhdc").val()+">"+$("#tthsdc").val()+">"+$("#tendv").val()+">"+$("#tenct").val()+">"+$("#quanly").val());	
}	
</script>
...
{	
	if ($madv[3]=="Tỉnh" && $_POST['xahuyen'] != '' && $_POST['hoten'] != "" && $_POST['ngaysinh'] != "" && $_POST['nguyenquan'] != "" && $_POST['truquan'] != "")
	{
		...
		$sql1=mysqli_query($con,$sql);$sotths=0;	
		...
		$sqlup = $sqlup."soquyetdinh='".$_POST['soquyetdinhcs']."', mtckv='".$_POST['mtckvcs']."', mtcdb='".$_POST['mtcdbcs']."', mtcht='".$_POST['mtchtcs']."', ";
		$sqlup = $sqlup."ngaymat='".$_POST['ngaymatcs']."', soso='".$_POST['sosocs']."', ghichu='".$_POST['ghichucs']."', ";
		$sqlup = $sqlup."loaidd='".$_POST['loaiddcs']."', namddlc='".$_POST['namddlccs']."', tongiao='".$_POST['tongiao']."',tinhhinhhn='".$_POST['tinhhinhhn']."',";
...
if (strrpos($sqlup,"where")<=0)
			$sqlup = $sqlup." where idhsbm=".$_POST['soghidc'];
		$kq=mysqli_query($con,$sqlup);
	}
}		
?>
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Hồ sơ bà mẹ Việt Nam anh hùng</h4>
		</div>
...
if(isset($_POST['timkiem'])){
	...
	if ($_POST['tenxatim'] != "")
		$sql="select * from hosobm where xa like '".$huyenxa[1]."%' and huyen like '".$huyenxa[0]."%' and mahobo like '%".$_POST['mabotim']."%' and mahotinh Like '%".$_POST['matinhtim']."%' and hoten Like '%".str_replace("_"," ",$_POST['hotentim'])."%'";
	else
	{
		if ($madv[3]=="Xã")
			$sql="select * from hosobm where mahobo like '%".$_POST['mabotim']."%' and mahotinh Like '%".$_POST['matinhtim']."%' and hoten Like '%".str_replace("_"," ",$_POST['hotentim'])."%' and xa='".$madv[0]."'";
		else if ($madv[3]=="Huyện")
			$sql="select * from hosobm where mahobo like '%".$_POST['mabotim']."%' and mahotinh Like '%".$_POST['matinhtim']."%' and hoten Like '%".str_replace("_"," ",$_POST['hotentim'])."%' and huyen='".$madv[0]."'";
		else
			$sql="select * from hosobm where mahobo like '%".$_POST['mabotim']."%' and mahotinh Like '%".$_POST['matinhtim']."%' and hoten Like '%".str_replace("_"," ",$_POST['hotentim'])."%'";
	}
	//echo $sql;
...
else
{
	...
	$huyenxa=explode(' - ',$tenhuyenxa);
	if ($ths[2] != "")
		$sql="select * from hosobm where xa like '".$huyenxa[1]."%' and huyen like '".$huyenxa[0]."%' and mahobo like '%".$ths[3]."%' and mahotinh Like '%".$ths[4]."%' and hoten Like '%".str_replace("_"," ",$ths[5])."%'";
	else
	{
		if ($madv[3]=="Xã")
			$sql="select * from hosobm where mahobo like '%".$ths[3]."%' and mahotinh Like '%".$ths[4]."%' and hoten Like '%".str_replace("_"," ",$ths[5])."%' and xa='".$madv[0]."'";
		else if ($madv[3]=="Huyện")
			$sql="select * from hosobm where mahobo like '%".$ths[3]."%' and mahotinh Like '%".$ths[4]."%' and hoten Like '%".str_replace("_"," ",$ths[5])."%' and huyen='".$madv[0]."'";
		else
			$sql="select * from hosobm where mahobo like '%".$ths[3]."%' and mahotinh Like '%".$ths[4]."%' and hoten Like '%".str_replace("_"," ",$ths[5])."%'";
	}
	$j = $ths[0];
	...
	while($row=mysqli_fetch_array($sql1)){					
		...
		$ls[$cs][8] = $row['bidanh'];
		$ls[$cs][9] = $row['soquyetdinh'];
		$ls[$cs][10] = $row['tths'];
		$ls[$cs][11] = $row['mtckv'];
		$ls[$cs][12] = $row['mtcdb'];
		$ls[$cs][13] = $row['mtcht'];
		$ls[$cs][14] = $row['ngaymat'];
		$ls[$cs][15] = $row['soso'];
		$ls[$cs][16] = $row['xa'];
		$ls[$cs][17] = $row['huyen'];
		$ls[$cs][18] = $row['ghichu'];		
		...
		$ls[$cs][21] = $row['filea'];
		$ls[$cs][22] = $row['dieuduong'];
		$ls[$cs][23] = $row['hinhthucddlt'];
		$ls[$cs][24] = $row['bhyt'];
		$ls[$cs][25] = $row['ngaydc'];
		$ls[$cs][26] = $row['noidc'];
		$ls[$cs][27] = $row['phanloaidc'];	
		$ls[$cs][28] = $row['filedk'];
		$ls[$cs][29] = $row['dantoc'];
		$ls[$cs][30] = $row['tongiao'];
		$ls[$cs][31] = $row['tinhhinhhn'];
		$cs = $cs + 1;
	$kq=$kq + 1;
	if ($kq == 5)
	{
