...
$j = $ahs[0];
	...
	if (strlen($_GET['id'])>=20)
		$sql="select * from ".ktnam($ahs[2])." where xacnhan='Duyệt' and huyen='".$madv[0]."' and xa Like '%".str_replace('_',' ',$ahs[1])."%'";
	else
		$sql="select * from ".ktnam(date('Y'))." where xacnhan='Duyệt' and huyen='".$madv[0]."'";
	//$sql="select * from ".ktnam(date('Y'))." where xacnhan='Duyệt' and huyen='".$madv[0]."' and (mathe='' or mathe is null)";
			...
			<div class="modal-body">				
					<form name="thoaitk" role='form' method='POST' class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK1" class="form-group">								
							...
							<div class="col-sm-9">								
								<select name="tenxatk" id="tenxatk" class="form-control">
									<?php
									$sqltcml = "Select xa.tenxa,huyen.tenhuyen From xa inner join huyen on xa.tthuyen=huyen.tthuyen Where huyen.tenhuyen='".$madv[0]."'";
									$qrtcml = mysqli_query($con,$sqltcml);			
									while($rtcml=@mysqli_fetch_array($qrtcml))
									{
							...
							<label for="field-3" class="col-sm-3 control-label">Năm bhyt cần tìm:</label>
							<div class="col-sm-9">
...
<script type="text/javascript">
	...
	function get_action2() {				
			return "dkbhyttheodoihuyen.php?id=1>"+$('#tenxatk').val()+">"+$('#namtk').val()+">cochucnangtimkiem";			
    }
</script>
<!------ FOOTER --------->
