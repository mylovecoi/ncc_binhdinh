...
<script type="text/javascript" language="javascript" class="init">
			...
			{  
			   ...
			   { 					
					...
					$("#idgiaytolscs").val($(this).find('td:nth-child(5)').text());
					$("#idhslscs").val($(this).find('td:nth-child(6)').text());					
			   }); 
			}
...
if(isset($_POST["capnhatts"]))
	{		
		$sqlup = "Update giaytokc Set tengiayto='".$_POST['tengiaytocs']."', chitiet='".$_POST['chitietcs']."', soluong=".$_POST['soluongcs'];		
		$sqlup = $sqlup." Where idgiaytokc=".$_POST['idgiaytolscs'];
		//echo $sqlup;	
		$kq=mysqli_query($con,$sqlup);
	}
if(isset($_POST["themmoi"]))
	{		
		$sqlup = "Insert into giaytokc Set tengiayto='".$_POST['tengiayto']."', chitiet='".$_POST['chitiet']."', soluong=".kieudouble($_POST['soluong']);		
		$sqlup = $sqlup.", idhskc=".$ihs;
		//echo $sqlup;	
		$kq=mysqli_query($con,$sqlup);
	}	
?>
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Danh sách giấy tờ hồ sơ người hoạt động kháng chiến 
			<a style = "float:right; color:rgb(255, 255, 225)" href='/Nhap/Nhaphosokc/nhaphosoncc.php?id=1>0'>
			Trở lại
			</a>
	<?php
		if ($madv[3]=="Tỉnh")
	...
	<?php		
		$sqlgt = "Select * From giaytokc Where idhskc = $ihs order by chitiet";	
		$i2 = 0;	
		...
		while($rctgt=@mysqli_fetch_array($qrctgt))
			{	
				...
				echo "<tr>";
					...
					echo "<td style='text-align: left; display:none'>".$rctgt['idgiaytokc']."</td>";
					echo "<td style='text-align: left; display:none'>".$rctgt['idhskc']."</td>";
				echo "</tr>";	
			}		
			echo "</tbody>";
	?>		
			...
			<div class="modal-body">				
					<form name="thoaics" role='form' method='POST' class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK" class="form-group">							
							...
							<div class="col-sm-10">
								<input name="idgiaytolscs" type="text" class="form-control" style="display:none" id="idgiaytolscs">
							</div>
							<div class="col-sm-10">
...
<script type="text/javascript">
    ...
    function get_action() {				
			return "nhapgiayto.php?id=" + $('#idhslscs').val();			
    }
