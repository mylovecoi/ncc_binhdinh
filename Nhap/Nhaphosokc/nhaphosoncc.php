...
{		
	window.location.href="/Reports/Phieudoichieukc.php?id="+$("#soghi").val()+">"+$("#hotendc").val()+">"+$("#ngaysinhdc").val()+">"+$("#nguyenquandc").val()+">"+$("#truquandc").val()+">"+$("#mahsbodc").val()+">"+$("#mahstinhdc").val()+">"+$("#tthsdc").val()+">"+$("#tendv").val()+">"+$("#tenct").val()+">"+$("#quanly").val();	
}	
</script>
...
<script type="text/javascript" language="javascript" class="init">
			...
			{  
			   ...
			   { 
					if ($(this).find('td:last').text() == "ncc" && $(this).find('td:nth-child(30)').text() == "")
					{			
						...
						$("#xaAPI").val($(this).find('td:nth-child(30)').text());
						$("#huyenAPI").val($(this).find('td:nth-child(31)').text());
					}
else
{
$("#soghidc").val($(this).find('td:nth-child(7)').text());
			...
			{  
			   ...
			   { 
					...
					{						
						...
						$("#mahosobocs").val($(this).find('td:nth-child(22)').text());	
						$("#mahosotinhcs").val($(this).find('td:nth-child(23)').text());
						$("#hotencs").val($(this).find('td:nth-child(2)').text());
						$("#bidanhcs").val($(this).find('td:nth-child(8)').text());
						...
						$("#tghoatdongcs").val($(this).find('td:nth-child(9)').text());
						$("#sonamcs").val($(this).find('td:nth-child(10)').text());
						$("#sothangcs").val($(this).find('td:nth-child(11)').text());
						$("#hhccpcs").val($(this).find('td:nth-child(12)').text());
						$("#hangcpcs").val($(this).find('td:nth-child(13)').text());
						$("#hhccmcs").val($(this).find('td:nth-child(14)').text());
						$("#hangcmcs").val($(this).find('td:nth-child(15)').text());
						$("#qdhhccs").val($(this).find('td:nth-child(16)').text());
						$("#qdtcmlcs").val($(this).find('td:nth-child(17)').text());
						$("#qdtcmtpcs").val($(this).find('td:nth-child(18)').text());
						$("#ngaymatcs").val($(this).find('td:nth-child(19)').text());						
						$("#idhskccs").val($(this).find('td:nth-child(7)').text());
...
if(isset($_POST["capnhatts"]))
{
	if ($madv[3]=="Tỉnh" && $_POST['hotencs'] != "" && $_POST['ngaysinhcs'] != "" && $_POST['nguyenquancs'] != "" && $_POST['truquancs'] != "")
	{
		...
		$maxaAPI = 0;
		$mahuyenAPI = 0;
		$sqltt = "Select hoten,ngaysinh,maxaAPI,mahuyenAPI from hosokc WHERE idhskc = ".$_POST['idhskccs'];
...
{	
	if ($madv[3]=="Tỉnh" && $_POST['xahuyen'] != '' && $_POST['hoten'] != "" && $_POST['ngaysinh'] != "" && $_POST['nguyenquan'] != "" && $_POST['truquan'] != "")
	{
		...
		//API
		$result = mysqli_query($con,"select MAX(idhskc) FROM hosokc");
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Hồ sơ người HĐKC làm nghĩa vụ quốc tế giải phóng dân tộc  <span style = "float:right">Tổng hồ sơ: <?php
			if ($ahs[2]=="1")
				echo $madv[13];
			else
				echo $madv[20];
...
<?php
$j = $ahs[0];$kq=0;$sotr=0;$sotrthieu=0;
if ($ahs[2]=="1")
{
	$sotr= (int)($madv[13]/5);
	$sotrthieu=$madv[13]/5;
...
$sotr=$sotr+1;
	...
	{
		if ($ahs[2]=="1")
			$sql="select * from hosokc Where idhskc > 20430 LIMIT ".(5*($j-1)).",5";
		else if ($ahs[2]=="0")
			$sql="select * from hosokc Where idhskc <= 20430 LIMIT ".(5*($j-1)).",5";
		else if ($ahs[2]=="")
			$sql="select * from hosokc LIMIT ".(5*($j-1)).",5";
	}
	else if ($madv[3]=="Huyện")
		$sql="select * from hosokc where huyen='$madv[0]'  LIMIT ".(5*($j-1)).",5";
	else if ($madv[3]=="Xã")
		$sql="select * from hosokc where xa='$madv[0]' and huyen='$madv[1]' LIMIT ".(5*($j-1)).",5";
	...
	while($row=mysqli_fetch_array($sql1)){
		...
		$ls[$cs][28] = $row['maxaAPI'];
		$ls[$cs][29] = $row['mahuyenAPI'];
		$cs = $cs + 1;	
	}
	//if ($j == 0)
	...
	return $kq;
			...
			for($i=0;$i<$cs;$i++)
			{
				...
				echo "<tr>";
					...
					{
						echo"<td>					
							<a href=\"xoa.php?id=".$ls[$i][21]."\"  title='Xóa hố sơ và thông tin liên quan' onclick=\"return confirm('Bạn muốn xóa hố sơ này?')\" class='btn btn-red '>
								</i>
								Xóa
							</a>
						</td>";
			...
			<div class="modal-body">				
					<form name="thoaimoi" role='form' method='POST' enctype="multipart/form-data" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK1" class="form-group">
							...
							<label for="field-3" class="col-sm-2 control-label">Tháng năm chống pháp:</label>
							<div class="col-sm-3">
								<input name="sothang" type="text" class="form-control" id="sothang">
							</div>
							<label for="field-3" class="col-sm-2 control-label">Tháng năm chống mỹ:</label>
							<div class="col-sm-2">
							...
							<label for="field-3" class="col-sm-2 control-label">Đợt giải quyết:</label>
							<div class="col-sm-10">
...
<script type="text/javascript">
    ...
    function get_action() {				
			return "nhaphosoncc.php?id=" + $('#trang').val() + ">" + $('#idhskccs').val();			
    }
