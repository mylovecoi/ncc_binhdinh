...
<script type="text/javascript" language="javascript" class="init">
			...
			{  
			   ...
			   { 					
				   ...
				   giatri = $(this).find('td:nth-child(22)').text()+"-"+$(this).find('td:nth-child(21)').text()+"-"+$(this).find('td:nth-child(23)').text()+"-"+$(this).find('td:nth-child(24)').text();
				   giatri = giatri.replace(/ /g,"_");
				   //alert(giatri);
				   $("#xahuyencs").val(giatri);
				   giatri = $(this).find('td:nth-child(25)').text()+"-"+$(this).find('td:nth-child(5)').text();
				   giatri = giatri.replace(/ /g,"_");
				   //alert(giatri);
				   $("#quanhecs").val(giatri);
			   }); 
...
if(isset($_POST["capnhatts"]))
{
	if ($madv[3]=="Tỉnh")
	{
		...
		$sqlup = "Update thannhancc Set hoten='".$_POST['hotencs']."', ngaysinh='".$_POST['ngaysinhcs']."', gioitinh='".$_POST['gioitinhcs']."', quanhe='".str_replace('_', ' ', $qhcs[1])."', ";
		$sqlup = $sqlup."truquan='".$_POST['truquancs']."', nguyenquan='".$_POST['nguyenquancs']."', loaitrocap='".$_POST['loaitrocapcs']."', matrocap='".$_POST['matrocapcs']."', ";
		...
		$sqlup = $sqlup."loaidieuduong='".$_POST['loaidieuduongcs']."', namddlc='".$_POST['namddlccs']."', ghichu='".$_POST['ghichucs']."', bhyt='".$_POST['bhytcs']."', xa='".str_replace('_',' ',$xhcs[1])."', huyen='".str_replace('_',' ',$xhcs[0])."',gioitinhAPI=$gioitinhAPI,maquanhe=$qhcs[0]";

		if ($_POST['fileanhcu'] != $fileanh)
			$sqlup = $sqlup.",filea='".$fileanh."'";		
		$sqlup = $sqlup." Where idthannhancc=".$_POST['idthannhantkncs'];
		//echo $sqlup;	
...
{	
	if ($madv[3]=="Tỉnh")
	{
		...
		$sqlup = "Insert into thannhancc Set hoten='".$_POST['hoten']."', ngaysinh='".$_POST['ngaysinh']."', gioitinh='".$_POST['gioitinh']."', quanhe='".str_replace('_', ' ', $qh[1])."', ";
		$sqlup = $sqlup."truquan='".$_POST['truquan']."', nguyenquan='".$_POST['nguyenquan']."', loaitrocap='".$_POST['loaitrocap']."', matrocap='".$_POST['matrocap']."', ";
		...
		$sqlup = $sqlup."loaidieuduong='".$_POST['loaidieuduong']."', namddlc='".$_POST['namddlc']."', ghichu='".$_POST['ghichu']."', idhscc =".$ihs[0].", filea='".$fileanh."', bhyt='".$_POST['bhyt']."', xa='".str_replace('_',' ',$xh[1])."', huyen='".str_replace('_',' ',$xh[0])."', tenncc='".$ihs[3]."'";
		$sqlup = $sqlup . ", maxaAPI='" . str_replace('_', ' ', $xh[2]) . "', mahuyenAPI='" . str_replace('_', ' ', $xh[3]) . "',gioitinhAPI=$gioitinhAPI,maquanhe=$qh[0]";
		//echo $sqlup;
		$kq=mysqli_query($con,$sqlup);
		//API
		$result = mysqli_query($con,"select MAX(idthannhancc) FROM thannhancc where idhscc = $ihs[0]");
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Danh sách thân nhân hồ sơ người có công 
	...
	<?php
	$sqltn = "Select * From thannhancc Where idhscc = $ihs[0]";	
	$i1 = 0;	
		...
		{	
			$i1 = $i1 + 1;
				echo "<tr>";
					...
					echo "<td style='text-align: left; display:none'>".$rcttn['idthannhancc']."</td>";
					echo "<td style='text-align: left; display:none'>".$rcttn['idhscc']."</td>";
			...
			{
				echo"<td>
							<a href=\"xoathannhan.php?id=".$rcttn['idthannhancc']."\"  title='Xóa hố sơ và thông tin liên quan' onclick=\"return confirm('Bạn muốn xóa hố sơ này?')\" class='btn btn-red '>
								</i>
								Xóa
							</a>
						</td>";
			...
			<div class="modal-body">				
					<form name="thoaimoi" role='form' method='POST' enctype="multipart/form-data" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK1" class="form-group">
							...
							<label for="field-3" class="col-sm-2 control-label">Mã trợ cấp:</label>
							<div class="col-sm-7">								
								<select name="matrocap" id="matrocap" class="form-control">
									<?php
									$sqltcml = "Select matrocap,tentrocap From dmtrocap where matrocap Like '%TN-CC%'";
									$qrtcml = mysqli_query($con,$sqltcml);			
									while($rtcml=@mysqli_fetch_array($qrtcml))
									{
										echo "<option value=".$rtcml['matrocap'].">".$rtcml['tentrocap']."</option>";
									}
									?>
									<option value="" selected = "selected"></option>
								</select>
							...
							<label for="field-3" class="col-sm-2 control-label">Số tiền:</label>
							<div class="col-sm-2">
							...
							<div class="col-sm-2">
								<input name="ngaythoihuong" type="text" class="form-control" data-mask="date" id="ngaythoihuong">
							</div>
			...
			<div class="modal-body">				
					<form name="thoaics" role='form' method='POST' enctype="multipart/form-data" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK" class="form-group">
							...
							<label for="field-3" class="col-sm-2 control-label">Mã trợ cấp:</label>
							<div class="col-sm-7">								
								<select name="matrocapcs" id="matrocapcs" class="form-control">
									<?php
									$sqltcml = "Select matrocap,tentrocap From dmtrocap where matrocap Like '%TN-CC%'";
									$qrtcml = mysqli_query($con,$sqltcml);			
									while($rtcml=@mysqli_fetch_array($qrtcml))
