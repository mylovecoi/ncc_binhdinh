...
require("$_SERVER[DOCUMENT_ROOT]/Main/general.php");
	$key=explode('>',$_GET["id"]);
	$sqltn = "select idthannhanbb from thannhanbb where idhsbb = $key[0]";
	...
	$xoa="DELETE FROM hosobb Where idhsbb=$key[0]";
	$tt=mysqli_query($con,$xoa);
	$xoa="DELETE FROM giaytobb Where idhsbb=$key[0]";
	$tt=mysqli_query($con,$xoa);
	$xoa="DELETE FROM thannhanbb Where idhsbb=$key[0]";
$tt=mysqli_query($con,$xoa);
	$data = array(
		"id_doituong"=> $key[0],
		"doituong"=> "benhbinh",
