...
{		
	window.location.href="/Reports/Phieudoichieubb.php?id="+$("#soghi").val()+">"+$("#hotendc").val()+">"+$("#ngaysinhdc").val()+">"+$("#nguyenquandc").val()+">"+$("#truquandc").val()+">"+$("#mahsbodc").val()+">"+$("#mahstinhdc").val()+">"+$("#tthsdc").val()+">"+$("#ketluansgknlddc").val()+">"+$("#tendv").val()+">"+$("#tenct").val()+">"+$("#quanly").val();	
}	
function showAjaxModaltl()
{		
	window.location.href="/Reports/TrichLucHSBB.php?id="+$("#soghi").val();	
}
//
...
<script type="text/javascript" language="javascript" class="init">
			...
			{  
			   ...
			   { 
					if ($(this).find('td:last').text() == "ncc" && $(this).find('td:nth-child(55)').text().indexOf('%') >=0)
					{
						...
						$("#xaAPI").val($(this).find('td:nth-child(55)').text());
						$("#huyenAPI").val($(this).find('td:nth-child(56)').text());
					}
					else
					{
						$("#soghidc").val($(this).find('td:nth-child(7)').text());
...
$("#phanloaidccs").val($(this).find('td:nth-child(47)').text());
						...
						$("#giaycnbbsongaycs").val($(this).find('td:nth-child(54)').text());
					}
			   }); 
			}
...
{	
	if ($madv[3]=="Tỉnh" && $_POST['hotencs'] != "" && $_POST['ngaysinhcs'] != "" && $_POST['quequancs'] != "" && $_POST['truquancs'] != "")
	{
		...
		$sqlup = $sqlup."donvi='".$_POST['donvics']."', ngayxn='".$_POST['ngayxncs']."', thoigianct='".$_POST['thoigianctcs']."', thoigianvkk='".$_POST['thoigianvkkcs']."', ngaybbvdt='".$_POST['ngaybbvdtcs']."', giaycnbbsongay='".$_POST['giaycnbbsongaycs']."',gioitinhAPI=$gioitinhAPI";
		...
		$maxaAPI = 0;
		$mahuyenAPI = 0;
		$sqltt = "Select hoten,ngaysinh,maxaAPI,mahuyenAPI from hosobb WHERE idhsbb = ".$_POST['idhsbbcs'];
...
{	
	if ($madv[3]=="Tỉnh" && $_POST['xahuyen'] != '' && $_POST['hoten'] != "" && $_POST['ngaysinh'] != "" && $_POST['quequan'] != "" && $_POST['truquan'] != "")
	{
		...
		$sqlup = $sqlup."donvi='".$_POST['donvi']."', ngayxn='".$_POST['ngayxn']."', thoigianct='".$_POST['thoigianct']."', thoigianvkk='".$_POST['thoigianvkk']."', ngaybbvdt='".$_POST['ngaybbvdt']."', giaycnbbsongay='".$_POST['giaycnbbsongay']."'";
		...
		//API
		$result = mysqli_query($con,"select MAX(idhsbb) FROM hosobb");
 ...
 <div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Hồ sơ bệnh binh <span style = "float:right">Tổng hồ sơ: <?php echo $madv[9];?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></h4>
		</div>
...
<?php
$ahs = explode('>',$_GET['id']);
$j = $ahs[0];$kq=0;$sotr=0;$sotr= (int)($madv[9]/5);$sotrthieu=$madv[9]/5;
		if ($sotrthieu-$sotr >0)
	$sotr=$sotr+1;
$sotr=$sotr+1;
	...
	$cs=0;
	if ($madv[3]=="Tỉnh")
		$sql="select * from hosobb LIMIT ".(5*($j-1)).",5";
	else if ($madv[3]=="Huyện")
		$sql="select * from hosobb where huyen='$madv[0]' LIMIT ".(5*($j-1)).",5";
	else if ($madv[3]=="Xã")
		$sql="select * from hosobb where xa='$madv[0]' and huyen='$madv[1]' LIMIT ".(5*($j-1)).",5";
	...
	while($row=mysqli_fetch_array($sql1)){
		...
		$ls[$cs][52] = $row['giaycnbbsongay'];
		$ls[$cs][53] = $row['maxaAPI'];
		$ls[$cs][54] = $row['mahuyenAPI'];
		$cs = $cs + 1;	
	}
	//if ($j == 0)
	...
	return $kq;
			...
			for($i=0;$i<$cs;$i++)
			{
				...
				echo "<tr>";
					...
					echo "<td style='text-align: left; display:none'>".$ls[$i][53]."</td>";
					echo "<td style='text-align: left; display:none'>".$ls[$i][54]."</td>";
					if ($madv[3]=="Tỉnh")
					{
						echo"<td>					
							<a href=\"xoa.php?id=".$ls[$i][38]."\"  title='Xóa hố sơ và thông tin liên quan' onclick=\"return confirm('Bạn muốn xóa hố sơ này?')\" class='btn btn-red '>
								</i>
								Xóa
							</a>
						</td>";
			...
			<div class="modal-body">				
					<form name="thoaimoi" role='form' method='POST' enctype="multipart/form-data" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK1" class="form-group">
							...
							<label for="field-3" class="col-sm-2 control-label">Ngày xuất ngũ:</label>
							<div class="col-sm-3">
								<input name="ngayxn" type="text" class="form-control" id="ngayxn">
							</div>
							<label for="field-3" class="col-sm-2 control-label">Đơn vị:</label>
							<div class="col-sm-2">
								<input name="donvikhinghichedo" type="text" class="form-control" id="donvikhinghichedo">
							</div>
							<label for="field-3" class="col-sm-2 control-label">Cấp bậc xuất ngũ:</label>
							<div class="col-sm-2">
							...
							<label for="field-3" class="col-sm-2 control-label">TG ở chiến trường:</label>
							<div class="col-sm-2">
								<input name="thoigianct" type="text" class="form-control" id="thoigianct">
							</div>
							<label for="field-3" class="col-sm-2 control-label">TG ở vùng khó khăn:</label>
							<div class="col-sm-2">
								<input name="thoigianvkk" type="text" class="form-control" id="thoigianvkk">
							</div>
							<label for="field-3" class="col-sm-2 control-label">Ngày bị bệnh:</label>
							<div class="col-sm-6">
								<input name="ngaybbvdt" type="text" class="form-control" data-mask="date" id="ngaybbvdt">
							</div>
							<label for="field-3" class="col-sm-2 control-label">GCN bệnh binh:</label>
							<div class="col-sm-10">
								<input name="giaycnbbsongay" type="text" class="form-control" id="giaycnbbsongay">
							</div>
			...
			<div class="modal-body">				
					<form name="thoaics" role='form' method='POST' enctype="multipart/form-data" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK" class="form-group">
							...
							<label for="field-3" class="col-sm-2 control-label">Ngày xuất ngũ:</label>
							<div class="col-sm-3">
								<input name="ngayxncs" type="text" class="form-control" id="ngayxncs">
							</div>
							<label for="field-3" class="col-sm-2 control-label">Đơn vị:</label>
							<div class="col-sm-2">
							...
							<label for="field-3" class="col-sm-2 control-label">TG ở chiến trường:</label>
							<div class="col-sm-2">
								<input name="thoigianctcs" type="text" class="form-control" id="thoigianctcs">
							</div>
							<label for="field-3" class="col-sm-2 control-label">TG ở vùng khó khăn:</label>
							<div class="col-sm-2">
								<input name="thoigianvkkcs" type="text" class="form-control" id="thoigianvkkcs">
							</div>
							<label for="field-3" class="col-sm-2 control-label">Ngày bị bệnh:</label>
							<div class="col-sm-6">
								<input name="ngaybbvdtcs" type="text" class="form-control" data-mask="date" id="ngaybbvdtcs">
							</div>
							<label for="field-3" class="col-sm-2 control-label">GCN bệnh binh:</label>
							<div class="col-sm-10">
								<input name="giaycnbbsongaycs" type="text" class="form-control" id="giaycnbbsongaycs">
							</div>
							<label for="field-3" class="col-sm-2 control-label">Thời kỳ:</label>
							<div class="col-sm-2">
...
<script type="text/javascript">
    ...
    function get_action() {				
			return "nhaphosoncc.php?id=" + $('#trang').val() + ">" + $('#idhsbbcs').val();			
    }
