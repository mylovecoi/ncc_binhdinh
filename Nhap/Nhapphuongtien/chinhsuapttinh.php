...
$(document).ready(function() {
  $('#trang').change(function() {
		...
		window.location.href="chinhsuapttinh.php?id="+this.value+">0";
  });
 });	
function xoa(bien)
{
...
{	
	...
	for($i=0;$i<$count;$i++){
		//echo $madv[0];
	$sql="Update phuongtien Set chuyen='Duyệt' Where idphuongtien=".$key[$i];
...
$j = $ahs[0];
	...
	{
		$sql="select * from phuongtien where chuyen='Chuyển 2' and xa like '%".str_replace('_',' ',$ahs[2])."%' and huyen Like '%".str_replace('_',' ',$ahs[1])."%'";
	}
	else
	{
		$sql="select * from phuongtien where chuyen='Chuyển 2'";
	}	
...
<script type="text/javascript">
	...
	function get_action2() {
			return "chinhsuapttinh.php?id=1>"+$('#tenhuyentim').val()+">"+$('#tenxatim').val();			
    }
</script>
<!------ FOOTER --------->
