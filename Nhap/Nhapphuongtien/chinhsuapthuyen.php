...
$(document).ready(function() {
  $('#trang').change(function() {
		...
		window.location.href="chinhsuapthuyen.php?id="+this.value+">0";
  });
 });	
function xoa(bien)
{
...
{	
	...
	for($i=0;$i<$count;$i++){
		//echo $madv[0];
	$sql="Update phuongtien Set chuyen='Chuyển 2' Where idphuongtien=".$key[$i];
...
if(isset($_POST["capnhat"]))
{
	...
	$sqlpt = $sqlpt."tinhtrangsk='".$_POST['tinhtrangsk']."', lydo='".$_POST['lydo']."', xa='".str_replace('_',' ',$_POST['tenxatao'])."', sosoql='".$_POST['sosoql']."', noiql='".$_POST['noiql']."'";
...
$j = 1;
	...
	$cs=0;
	$sql="select * from phuongtien where huyen='".$madv[0]."' and chuyen='Chuyển 1'";
			...
			<div class="modal-body">				
					<form name="thoaichuyen" role='form' method='POST' enctype="multipart/form-data" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK" class="form-group">
							...
							<input type="submit" name="chuyentinh" class="btn btn-blue" value = "Cập nhật" id = "hhhh1">							
					</form>						
			</div>
		</div>
	</div>
