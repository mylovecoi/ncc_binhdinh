...
$(document).ready(function() {
  $('#trang').change(function() {
		...
		window.location.href="danhsachthutuchuyen.php?id="+this.value+">0";
  });
 });
...
{	
	...
	for($i=0;$i<$count;$i++){
		...
		if ($kytu[1]=="Chuyển 1")
		{
			$sql="Update thutuchc Set trangthai='Chuyển 2' Where ttthutuchc=".$kytu[0];
...
{	
	if ($_POST['trangthai']=="Đang xử lý huyện" || $_POST['trangthai']=="Bị trả lại")
	{
		if ($_POST['tralai']!="")
		{			
			$sqlpt = "Update thutuchc Set tralai='".$_POST['tralai']."', trangthai='Trả lại 1'";
...
if(isset($_POST["trahosox"]))
{	
	if ($_POST['trangthaix']=="Đang xử lý huyện" || $_POST['trangthaix']=="Bị trả lại")
	{
		if ($_POST['tralaix']!="")
		{			
			$sqlpt = "Update thutuchc Set tralai='".$_POST['tralaix']."', trangthai='Trả lại 1'";
...
if(isset($_POST["capnhat"]))
{
	if ($_POST['trangthai']=="Đang xử lý huyện" || $_POST['trangthai']=="Bị trả lại")
	{
		...
		$sqlpt = $sqlpt."loaidoituong='".$_POST['loaidoituong']."', trangthai='Chuyển 1', ngaylaphs='".doingay1($_POST['ngaylaphs'])."', tralai='".$_POST['tralai']."', phanloaihoso='".$_POST['phanloaihoso']."'";
		$sqlpt = $sqlpt." Where ttthutuchc=".$_POST['ttthutuchc'];
		$kqpt=mysqli_query($con,$sqlpt);
...
if(isset($_POST["capnhatx"]))
{
	if ($_POST['trangthaix']=="Đang xử lý huyện" || $_POST['trangthaix']=="Bị trả lại")
	{
		$sqlpt = "Update thutuchc Set phanloai='".$_POST['phanloaix']."', hoten='".$_POST['hotenx']."', ngaysinh='".$_POST['ngaysinhx']."', ";
		$sqlpt = $sqlpt."gioitinh='".$_POST['gioitinhx']."', nguyenquan='".$_POST['nguyenquanx']."', truquan='".$_POST['truquanx']."', noidung='".$_POST['noidungx']."', ";
		$sqlpt = $sqlpt."loaidoituong='".$_POST['loaidoituongx']."', trangthai='Chuyển 1', ngaylaphs='".doingay1($_POST['ngaylaphsx'])."', tralai='".$_POST['tralaix']."', ";
...
{	
	if ($_POST['trangthai1']=="Đang xử lý huyện" || $_POST['trangthai1']=="Bị trả lại")
	{
...
<form role='form' method='POST' class='form-horizontal form-groups-bordered'>				
		...
		<a style = "float:right; background-color:rgb(83, 181, 166)" onclick="showAjaxModalctim();" class='btn btn-success'>
			<i class='entypo-pencil'></i>Tìm kiểm
		</a>
...
if (strlen($ahs) > 6)
{
	...
	{
		$sql="select * from thutuchc where huyen='".$madv[0]."' and xa Like '%".str_replace('_',' ',$kiemtra[1])."%' and ngaylaphs between '".doingay1($kiemtra[2])."' and '".doingay1($kiemtra[3])."' and (trangthai='Chuyển 1' or trangthai='Chuyển 2' or trangthai='Trả lại 2' or trangthai='Duyệt')";
	}
	else
	{
		$sql="select * from thutuchc where huyen='".$madv[0]."' and xa Like '%".str_replace('_',' ',$kiemtra[1])."%' and (trangthai='Chuyển 1' or trangthai='Chuyển 2' or trangthai='Trả lại 2' or trangthai='Duyệt')";
	}
}
else
{
	$sql="select * from thutuchc where huyen='".$madv[0]."' and (trangthai='Chuyển 1' or trangthai='Chuyển 2' or trangthai='Trả lại 2' or trangthai='Duyệt')";
	$j = $ahs;
}
//echo $sql;
...
<form name="thoaimoi" role='form' method='POST' class='form-horizontal form-groups-bordered'>
	...
	<h3><strong>Thông tin về đối tượng xin hưởng trợ cấp:</strong><button type="button" style = "float:right" class="btn btn-black" data-dismiss="modal">Đóng</button><input type="submit" name="capnhat" class="btn btn-info" value = "Cập nhật" style = "float:right" id = "hhhh"><input type="submit" name="trahoso" class="btn btn-info" value = "Trả hồ sơ về xã" style = "float:right"></h3>
	<legend></legend>
...
<form name="thoaimoix" role='form' method='POST' class='form-horizontal form-groups-bordered'>
	...
	<h3><strong>Thông tin về đối tượng xin hưởng trợ cấp:</strong><button type="button" style = "float:right" class="btn btn-black" data-dismiss="modal">Đóng</button><input type="submit" name="capnhatx" class="btn btn-info" value = "Cập nhật" style = "float:right" id = "hhhhx"><input type="submit" name="trahosox" class="btn btn-info" value = "Trả hồ sơ về xã" style = "float:right"></h3>
	<legend></legend>
...
<div class="modal fade" id="modal-5">
	<div class="modal-dialog">
		<div class="modal-content">				
			<div class="modal-body">				
					<form name="thoaitao" role='form' method='POST' action="/Thoaikx/Danhsachhosodgqhuyen.php" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK1" class="form-group">
							...
							<label for="field-3" class="col-sm-3 control-label">Ngày nộp hồ sơ từ:</label>
							<div class="col-sm-9">
								<input name="Tu" type="text" class="form-control" data-mask="date" id="Tu">
							</div>
							<label for="field-3" class="col-sm-3 control-label">Ngày nộp hồ sơ đến:</label>
							<div class="col-sm-9">
...
<script type="text/javascript">
	...
	function get_action2() {
			return "danhsachthutuchuyen.php?id=1>"+$('#tenhuyentim').val()+">"+$('#ngaylaphstu').val()+">"+$('#ngaylaphsden').val();			
    }
</script>	
<!------ FOOTER --------->
