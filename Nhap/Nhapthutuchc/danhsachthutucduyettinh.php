...
$(document).ready(function() {
  $('#trang').change(function() {
		...
		window.location.href="danhsachthutucduyettinh.php?id="+this.value+">0";
  });
 });
...
{
	if ($_POST['trangthai']=="Chờ duyệt hồ sơ")
	{
		$sqlpt = "Update thutuchc Set quyetdinh='".$_POST['quyetdinh']."', ngayqd='".doingay1($_POST['ngayqd'])."', ngayhuong='".doingay1($_POST['ngayhuong'])."', ";
		$sqlpt = $sqlpt."phanloaitc='".$_POST['phanloaitc']."', muctrocap=".str_replace(',','',str_replace('.','',$_POST['muctrocap'])).", phucap=".str_replace(',','',str_replace('.','',$_POST['phucap'])).", truylinh=".str_replace(',','',str_replace('.','',$_POST['truylinh']));
		$sqlpt = $sqlpt.",trangthai='Duyệt'";
...
{
	if ($_POST['trangthaix']=="Chờ duyệt hồ sơ")
	{
		$sqlpt = "Update thutuchc Set quyetdinh='".$_POST['quyetdinhx']."', ngayqd='".doingay1($_POST['ngayqdx'])."', ngayhuong='".doingay1($_POST['ngayhuongx'])."', ";
		$sqlpt = $sqlpt."phanloaitc='".$_POST['phanloaitcx']."', muctrocap=".str_replace(',','',str_replace('.','',$_POST['muctrocapx'])).", phucap=".str_replace(',','',str_replace('.','',$_POST['phucapx'])).", truylinh=".str_replace(',','',str_replace('.','',$_POST['truylinhx']));
		$sqlpt = $sqlpt.",trangthai='Duyệt'";
...
{	
	if ($_POST['trangthai']=="Chờ duyệt hồ sơ")
	{
		if ($_POST['tralai']!="")
		{			
			$sqlpt = "Update thutuchc Set tralai='".$_POST['tralai']."', trangthai='Trả lại 2'";
			$sqlpt = $sqlpt." Where ttthutuchc=".$_POST['ttthutuchc'];
			//echo $sqlpt;
			$kqpt=mysqli_query($con,$sqlpt);
		}
...
{	
	if ($_POST['trangthaix']=="Chờ duyệt hồ sơ")
	{
		if ($_POST['tralaix']!="")
		{			
			$sqlpt = "Update thutuchc Set tralai='".$_POST['tralaix']."', trangthai='Trả lại 2'";
			$sqlpt = $sqlpt." Where ttthutuchc=".$_POST['ttthutuchcx'];
			//echo $sqlpt;
			$kqpt=mysqli_query($con,$sqlpt);
		}
...
<form role='form' method='POST' class='form-horizontal form-groups-bordered'>
		...
		<a style = "float:right; background-color:rgb(83, 181, 166)" onclick="showAjaxModalccd();" class='btn btn-success'>
			<i class='entypo-pencil'></i>In hồ sơ chờ duyệt
		</a>
...
if (strlen($ahs) > 3)
{
	...
	{
		$sql="select * from thutuchc where trangthai='Chuyển 2' and ngaylaphs between '".doingay1($kiemtra[2])."' and '".doingay1($kiemtra[3])."' and huyen Like '%".str_replace('_',' ',$kiemtra[1])."%'";
	}
	else
	{		
		$sql="select * from thutuchc where trangthai='Chuyển 2' and huyen Like '%".str_replace('_',' ',$kiemtra[1])."%'";
	}
}
else
{
	$sql="select * from thutuchc where trangthai='Chuyển 2'";
	$j = $ahs;
}
//echo $sql;
...
<?php
			for($i=($j - 1)*15;$i<15*$j;$i++)
			{
				...
				echo "<tr>";					
					...
					if($ls[$i][20]=="Chuyển 2")
						echo "<td style='text-align: left;'>Chờ duyệt hồ sơ</td>";					
...
<form name="thoaimoi" role='form' method='POST' class='form-horizontal form-groups-bordered'>
	...
	<label for="field-3" class="col-sm-2 control-label">Nội dung Trả hồ sơ:</label>
	<div class="col-sm-10">
...
<div class="modal fade" id="modal-5">
	<div class="modal-dialog">
		<div class="modal-content">				
			<div class="modal-body">				
					<form name="thoaitao" role='form' method='POST' action="/Thoaikx/Danhsachhosochoduyet.php" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK1" class="form-group">
							<label for="field-3" class="col-sm-3 control-label">Tên Huyện:</label>
							<div class="col-sm-9">								
...
<script type="text/javascript">
	...
	function get_action2() {
			return "danhsachthutucduyettinh.php?id=1>"+$('#tenhuyentim').val()+">"+$('#ngaylaphstu').val()+">"+$('#ngaylaphsden').val();			
    }
</script>	
<!------ FOOTER --------->
