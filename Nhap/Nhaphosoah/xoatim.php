...
require("$_SERVER[DOCUMENT_ROOT]/Main/general.php");
	$key=explode('>',$_GET["id"]);
	$sqltn = "select idthannhanah from thannhanah where idhsbm = $key[0]";
	...
	$xoa="DELETE FROM hosoah Where idhsah=$key[0]";
	$tt=mysqli_query($con,$xoa);
	$xoa="DELETE FROM giaytoah Where idhsah=$key[0]";
	$tt=mysqli_query($con,$xoa);
	$xoa="DELETE FROM thannhanah Where idhsah=$key[0]";
	$tt=mysqli_query($con,$xoa);
	$data = array(
		"id_doituong"=> $key[0],
		"doituong"=> "anhhung",
