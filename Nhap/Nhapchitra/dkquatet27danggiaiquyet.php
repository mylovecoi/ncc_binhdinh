...
$(document).ready(function() {
  $('#trang').change(function() {
		...
		window.location.href="dkquatet27danggiaiquyet.php?id="+this.value+">0";
  });
 });
</script>
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Danh sách hồ sơ hưởng quà tết, 27/7 đang giải quyết</h4>
		</div>
...
if (strlen($_GET['id'])<8)
{
	$sql="select * from quatet27 where chuyen='Chuyển' and huyen='".$madv[0]."' and namqua Like '%".$ahs[1]."%'";
}
else
{
	$sql="select * from quatet27 where chuyen='Chuyển' and huyen='".$madv[0]."' and namqua Like '".$ahs[3]."%' and loaidoituong Like '".$ahs[1]."%' and mucqua=".str_replace(',','',str_replace('.','',$ahs[2]))." and xa like '".$ahs[4]."%'";
}
		...
		<table class="table table-hover table-striped table-bordered table-advanced tablesorter display" id="table-2">
			<thead>
				<tr>					
					...
					<th style="background-color:rgb(54, 169, 224);" width="8%"><strong style='color:rgb(255, 255, 225)'>Trạng thái</strong></th>
	...
	return $kq;
			...
			for($i=($j - 1)*15;$i<15*$j;$i++)
			{
				...
				if ($ls[$i][2]=="lt")		
					...
					if ($ls[$i][7]=="Chuyển")
						echo "<td style='text-align: left;'>Đang giải quyết</td>";					
...
<script type="text/javascript">
	...
	function get_action1() {				
			return "dkquatet27danggiaiquyet.php?id="+"1>"+$('#namqua').val();			
    }
	function get_action2() {				
			return "dkquatet27danggiaiquyet.php?id=1>"+$('#namquatao').val();			
    }
	function get_action3() {				
			return "dkquatet27danggiaiquyet.php?id=1>"+$('#loaidoituongtim').val()+">"+$('#mucquatim').val()+">"+$('#namquatim').val()+">"+$('#xatim').val();			
    }
	function get_action4() {				
			return "danhsachquadgq.php";
    }	
</script>
<!------ FOOTER --------->
