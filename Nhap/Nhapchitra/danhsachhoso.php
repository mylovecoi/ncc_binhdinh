...
$cs=0;
	...
	while($row=mysqli_fetch_array($sql1)){					
		...
		$ls[$cs][1] = $row['truquan'];
		$ls[$cs][2] = 'Hàng tháng';
		$ls[$cs][3] = 'Anh hùng vũ trang và Anh hùng lao động';
		$ls[$cs][4] = '1,361,000';
		$cs = $cs + 1;
	$kq=$kq + 1;
	if ($kq == 15)
		...
		<table class="table table-hover table-striped table-bordered table-advanced tablesorter display" id="table-2">
			<thead>
				<tr>					
					...
					<th style="background-color:rgb(54, 169, 224)" width="15%"><strong style='color:rgb(255, 255, 225)'>Thông tin hồ sơ</strong></th>
					<th style="background-color:rgb(54, 169, 224)" width="23%"><strong style='color:rgb(255, 255, 225)'>Địa chỉ</strong></th>
					<th style="background-color:rgb(54, 169, 224)" width="25%"><strong style='color:rgb(255, 255, 225)'>Loại trợ cấp</strong></th>
					<th style="background-color:rgb(54, 169, 224)" width="10%"><strong style='color:rgb(255, 255, 225)'>Tên trợ cấp</strong></th>
	...
	return $kq;
			...
			for($i=($j - 1)*15;$i<15*$j;$i++)
			{
				...
				echo "<tr>";					
					...
					echo"<td>
							<a href=\"xoa.php?id=".$ls[$i][0]."\"  title='Xóa hố sơ chi trả' onclick=\"return confirm('Bạn muốn xóa hố sơ này?')\" class='btn btn-red '>
								</i>
								Xóa
							</a>
						</td>";
