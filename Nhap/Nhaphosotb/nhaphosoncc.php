...
{		
	window.location.href="/Reports/Phieudoichieutb.php?id="+$("#soghi").val()+">"+$("#hotendc").val()+">"+$("#ngaysinhdc").val()+">"+$("#nguyenquandc").val()+">"+$("#truquandc").val()+">"+$("#mahsbodc").val()+">"+$("#mahstinhdc").val()+">"+$("#tthsdc").val()+">"+$("#ketluansgknlddc").val()+">"+$("#tendv").val()+">"+$("#tenct").val()+">"+$("#quanly").val();	
}
function showAjaxModaltl()
{		
	window.location.href="/Reports/TrichLucHSTB.php?id="+$("#soghi").val();	
}	
</script>
...
{	
	if ($madv[3]=="Tỉnh" && $_POST['hotencs'] != "" && $_POST['ngaysinhcs'] != "" && $_POST['quequancs'] != "" && $_POST['truquancs'] != "")
	{
		...
		$maxaAPI = 0;
		$mahuyenAPI = 0;
		$sqltt = "Select hoten,ngaysinh,maxaAPI,mahuyenAPI from hosotb WHERE idhstb = ".$_POST['idhstbcs'];
...
if(isset($_POST["themmoi"]))
{
	if ($madv[3]=="Tỉnh" && $_POST['xahuyen'] != '' && $_POST['hoten'] != "" && $_POST['ngaysinh'] != "" && $_POST['quequan'] != "" && $_POST['truquan'] != "")
	{
		...
		//API
		$result = mysqli_query($con,"select MAX(idhstb) FROM hosotb");
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Hồ sơ thương binh <span style = "float:right">Tổng hồ sơ: <?php echo $madv[16];?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></h4>
		</div>
...
<?php
$ahs = explode('>',$_GET['id']);
$j = $ahs[0];$kq=0;$sotr=0;$sotr= (int)($madv[16]/5);$sotrthieu=$madv[16]/5;
		if ($sotrthieu-$sotr >0)
	$sotr=$sotr+1;
$sotr=$sotr+1;
	...
	$cs=0;
	if ($madv[3]=="Tỉnh")
		$sql="select * from hosotb LIMIT ".(5*($j-1)).",5";
	else if ($madv[3]=="Huyện")
		$sql="select * from hosotb where huyen='$madv[0]' LIMIT ".(5*($j-1)).",5";
	else if ($madv[3]=="Xã")
		$sql="select * from hosotb where xa='$madv[0]' and huyen='$madv[1]' LIMIT ".(5*($j-1)).",5";
	...
	return $kq;
			...
			for($i=0;$i<$cs;$i++)
			{
				...
				echo "<tr>";
					...
					{
						echo"<td>					
							<a href=\"xoa.php?id=".$ls[$i][44]."\"  title='Xóa hố sơ và thông tin liên quan' onclick=\"return confirm('Bạn muốn xóa hố sơ này?')\" class='btn btn-red '>
								</i>
								Xóa
							</a>
						</td>";
						...
						<div id = "sTTKK1" class="form-group">
							...
							<div class="col-sm-6">
								<select name="noidungcs" id="noidungcs" class="form-control">
									<?php
									$sql = "select tentrocap, tileden from dmtrocap where matrocap like 'TC-TB%'";
...
<script type="text/javascript">
    ...
    function get_action() {				
			return "nhaphosoncc.php?id=" + $('#trang').val() + ">" + $('#idhstbcs').val();			
    }
	function get_action1() {				
			return "nhaphosoncc.php?id=1>0>1";
    }
