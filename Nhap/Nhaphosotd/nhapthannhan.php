...
if(isset($_POST["capnhatts"]))
{
	if ($madv[3]=="Tỉnh")
	{
		...
		$sqlup = "Update thannhantd Set hoten='".$_POST['hotencs']."', ngaysinh='".$_POST['ngaysinhcs']."', gioitinh='".$_POST['gioitinhcs']."', quanhe='".str_replace('_', ' ', $qhcs[1])."', ";
		...
		$sqlup = $sqlup." Where idthannhantd=".$_POST['idthannhantkncs'];
		//echo $sqlup;	
...
{	
	if ($madv[3]=="Tỉnh")
	{
		...
		$sqlup = "Insert into thannhantd Set hoten='".$_POST['hoten']."', ngaysinh='".$_POST['ngaysinh']."', gioitinh='".$_POST['gioitinh']."', quanhe='".str_replace('_', ' ', $qh[1])."', ";
		...
		$sqlup = $sqlup."loaidieuduong='".$_POST['loaidieuduong']."', namddlc='".$_POST['namddlc']."', ghichu='".$_POST['ghichu']."', idhstd =".$ihs[0].", filea='".$fileanh."', bhyt='".$_POST['bhyt']."', xa='".str_replace('_',' ',$xh[1])."', huyen='".str_replace('_',' ',$xh[0])."', tenncc='".$ihs[3]."'";
		$sqlup = $sqlup . ", maxaAPI='" . str_replace('_', ' ', $xh[2]) . "', mahuyenAPI='" . str_replace('_', ' ', $xh[3]) . "',gioitinhAPI=$gioitinhAPI,maquanhe=$qh[0]";
		$kq=mysqli_query($con,$sqlup);
		//API
		$result = mysqli_query($con,"select MAX(idthannhantd) FROM thannhantd where idhstd = $ihs[0]");
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Danh sách thân nhân hồ sơ địch bắt tù đày 
			<a style = "float:right; color:rgb(255, 255, 225)" href='/Nhap/Nhaphosotd/nhaphosoncc.php?id=1>0'>
			Trở lại
			</a>
	<?php
		if ($madv[3]=="Tỉnh")
	...
	<?php
	$sqltn = "Select * From thannhantd Where idhstd = $ihs[0]";	
	$i1 = 0;	
		...
		{	
			$i1 = $i1 + 1;
				echo "<tr>";
					...
					echo "<td style='text-align: left; display:none'>".$rcttn['idthannhantd']."</td>";
					echo "<td style='text-align: left; display:none'>".$rcttn['idhstd']."</td>";
			...
			{
				echo"<td>
							<a href=\"xoathannhan.php?id=".$rcttn['idthannhantd']."\"  title='Xóa hố sơ và thông tin liên quan' onclick=\"return confirm('Bạn muốn xóa hố sơ này?')\" class='btn btn-red '>
								</i>
								Xóa
							</a>
						</td>";
						...
						<div id = "sTTKK1" class="form-group">
							...
							<div class="col-sm-7">								
								<select name="matrocap" id="matrocap" class="form-control">
									<?php
									$sqltcml = "Select matrocap,tentrocap From dmtrocap where matrocap Like '%TN-TD%'";
