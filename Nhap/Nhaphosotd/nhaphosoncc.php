...
{		
	window.location.href="/Reports/Phieudoichieutd.php?id="+$("#soghi").val()+">"+$("#hotendc").val()+">"+$("#ngaysinhdc").val()+">"+$("#nguyenquandc").val()+">"+$("#truquandc").val()+">"+$("#mahsbodc").val()+">"+$("#mahstinhdc").val()+">"+$("#tthsdc").val()+">"+$("#tendv").val()+">"+$("#tenct").val()+">"+$("#quanly").val();	
}	
</script>
...
$(document).ready(function() {
  $('#trang').change(function() {
		...
		window.location.href="nhaphosoncc.php?id="+this.value+">0>"+$("#loai").val();
  });
 });		
</script>
<script>
...
{	
	if ($madv[3]=="Tỉnh" && $_POST['hotencs'] != "" && $_POST['ngaysinhcs'] != "" && $_POST['nguyenquancs'] != "" && $_POST['truquancs'] != "")
	{
		...
		if ($ahs[0]=="1")
			$sqlup = $sqlup.",loaiquatet='1 lần'";
		else
			$sqlup = $sqlup.",loaiquatet='Hàng tháng'";
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Hồ sơ người HĐKC bị địch bắt, tù đày <span style = "float:right">Tổng hồ sơ: <?php
...
?>
		...
		<input name="loai" type="text" class="form-control" style="display:none" id="loai" value="<?php echo $ahs[2]; ?>">
...
$j = $ahs[0];
	...
	{
		if ($ahs[2]=="1")
			$sql="select * from hosotd Where loaiquatet='1 lần'";
		else if ($ahs[2]=="0")
			$sql="select * from hosotd Where loaiquatet='Hàng tháng'";
		else if ($ahs[2]=="")
			$sql="select * from hosotd";
	}
	else if ($madv[3]=="Huyện")
		$sql="select * from hosotd where huyen='$madv[0]'";
	else if ($madv[3]=="Xã")
		$sql="select * from hosotd where xa='$madv[0]' and huyen='$madv[1]' order by tths";
	//$sql="select * from hosotd";
			...
			<div class="modal-body">				
					<form name="thoaimoi" role='form' method='POST' enctype="multipart/form-data" class='form-horizontal form-groups-bordered'>
						<div id = "sTTKK1" class="form-group">
							...
							<input name="pl" type="text" class="form-control" style="display: none" id="pl" value = "<?php echo $ahs[2]; ?>">
							</div>
		...
		<div class="modal-content">
			<div class="modal-body">				
					<form name="thoaics" role='form' method='POST' enctype="multipart/form-data" class='form-horizontal form-groups-bordered'>
						...
						<input name="plcs" type="text" class="form-control" style="display: none" id="plcs" value = "<?php echo $ahs[2]; ?>">
							<button type="button" class="btn btn-black" data-dismiss="modal">Đóng</button>
					<?php		
						if ($madv[3]=="Tỉnh")
...
<script type="text/javascript">
    ...
    function get_action() {				
			return "nhaphosoncc.php?id=" + $('#trang').val() + ">" + $('#idhstdcs').val()+">"+$('#plcs').val();
    }
	function get_action1() {				
			return "nhaphosoncc.php?id="+$('#trangm').val()+">0>"+$('#pl').val();
    }
