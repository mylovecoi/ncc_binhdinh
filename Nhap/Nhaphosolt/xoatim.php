...
require("$_SERVER[DOCUMENT_ROOT]/Main/general.php");
	$key=explode('>',$_GET["id"]);
$sqltn = "select idthannhanlt from thannhanlt where idhslt = $key[0]";
...
while($row = mysqli_fetch_array($qrsql)){
	...
	$xoa="DELETE FROM hosolt Where idhslt=$key[0]";
	$tt=mysqli_query($con,$xoa);
	$xoa="DELETE FROM giaytolt Where idhslt=$key[0]";
	$tt=mysqli_query($con,$xoa);
	$xoa="DELETE FROM thannhanlt Where idhslt=$key[0]";
	$tt=mysqli_query($con,$xoa);
	$data = array(
		"id_doituong"=> $key[0],
		"doituong"=> "tienkhangchien",
