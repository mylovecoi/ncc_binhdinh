...
if(isset($_POST["capnhatts"]))
	{		
		$sqlup = "Update giaytolt Set tengiayto='".$_POST['tengiaytocs']."', chitiet='".$_POST['chitietcs']."', soluong=".kieudouble($_POST['soluongcs']);		
		$sqlup = $sqlup." Where idgiaytolt=".$_POST['idgiaytotkncs'];
		//echo $sqlup;	
		$kq=mysqli_query($con,$sqlup);
	}
if(isset($_POST["themmoi"]))
	{		
		$sqlup = "Insert into giaytolt Set tengiayto='".$_POST['tengiayto']."', chitiet='".$_POST['chitiet']."', soluong=".kieudouble($_POST['soluong']);		
		$sqlup = $sqlup.", idhslt=".$ihs;
		//echo $sqlup;	
		$kq=mysqli_query($con,$sqlup);
	}	
?>
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title">Danh sách giấy tờ hồ sơ hoạt động trước 01/01/1945 
			<a style = "float:right; color:rgb(255, 255, 225)" href='/Nhap/Nhaphosolt/nhaphosoncc.php?id=1>0'>
			Trở lại
			</a>
	<?php
		if ($madv[3]=="Tỉnh")
	...
	<?php		
		$sqlgt = "Select * From giaytolt Where idhslt = $ihs order by chitiet";	
		$i2 = 0;	
		...
		while($rctgt=@mysqli_fetch_array($qrctgt))
			{	
				...
				echo "<tr>";
					...
					echo "<td style='text-align: left; display:none'>".$rctgt['idgiaytolt']."</td>";
					echo "<td style='text-align: left; display:none'>".$rctgt['idhslt']."</td>";
				echo "</tr>";	
			}		
			echo "</tbody>";
	?>		
